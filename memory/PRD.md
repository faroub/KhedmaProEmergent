...
- **Tests** `backend/tests/test_iter17_checkin_on_my_way.py` (15 tests): in_progress transition + `arrived_at`, automatic arrival chat message (single, correct sender/recipient, thread preview), guards (double check-in 400, pending→in_progress 400, client 403, other provider 403, unverified 403, bad enum 422), in_progress→awaiting_confirmation→completed, phone reveal during in_progress, guest booking check-in has no chat message, "On my way" REST message delivery/order/404/422. Full suite: 194 passed, 2 skipped.

## Iteration 19 — Client ETA countdown + job timer
- **`POST /api/bookings/{id}/eta {minutes 1..240}`** (provider, confirmed bookings only): stores `eta_minutes`, `eta_sent_at`, `eta_arrival_at`; posts the "🚗 I'm on my way…" chat line server-side (shared `_post_chat_message` helper, guests skipped) and pushes "Provider on the way". Frontend `api.sendEta()`; `TodayJobsCard` uses it and shows the sent state from server data.
- **Client ETA banner** (`TodayVisitCard`): live "Provider arriving in N min" (ticks every second, from `eta_arrival_at`), "arriving any moment now" within a 30-min grace window after the promised time; only while the booking is still `confirmed`.
- **Job timer**: both `TodayJobsCard` (provider, with "≈ X DZD so far (rate DZD/h)" for hourly jobs using `user.hourly_rate`) and `TodayVisitCard` (client) show a live HH:MM:SS counter from `arrived_at` while `in_progress`. On provider mark-done the server stores `worked_minutes = ceil(elapsed)`; for `rate_type=hourly` it sets `final_total_dzd = round(hourly_rate × minutes / 60)` and overwrites `estimated_total` (exact billing). Bookings list shows "Worked 1h 15m" and drops the "≈" for final totals.
- Client bookings tab polls every 60 s while focused. Native SQLite cache gained an `extra` JSON column (`LIVE_FIELDS` in `schema.ts`) so live fields survive the offline store; web (AsyncStorage) keeps full objects. Shared `src/hooks/useNow.ts` (`useNow`, `formatClock`, `formatMinutes`).
- Tests: +6 in `test_iter17_checkin_on_my_way.py` (ETA fields/chat, update + 422 range, guards 403/404/400, hourly exact billing, task price kept, no timer without check-in). Suite: 200 passed, 2 skipped.
